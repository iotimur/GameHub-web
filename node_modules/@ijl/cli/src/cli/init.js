        ...
        for (const fileOrDir of files) {
            if (fileOrDir.isDirectory()) {
                ...
                try {
                    ...
                    logWarning(`${cleanFileName} Already exists, skip this step.`);
                } catch (error) {
                    ...
                    logSuccess(`${cleanFileName} successfully created.`)
                }
            }
        }
    } catch (error) {
...
const main = async (advanced) => {
    ...
    try {
        ...
        logSuccess('Updating package.json ...')
        await writeFile(packagePath + '.json', JSON.stringify(package, null, 4));

        logSuccess('Installing the required dependencies ...')
        require('child_process').execSync(
            `npm i --save react@18 react-dom@18 express typescript @types/react@18 @types/react-dom@18 ${advanced ? 'react-router-dom' : ''}`, {
                stdio: 'inherit'
            }
        );
